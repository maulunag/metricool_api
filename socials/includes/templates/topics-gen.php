...
<div class="mb-6 flex items-center justify-between flex-wrap gap-4">
    <div>
        ...
        <p class="text-gray-500 text-sm"><span class="text-amber-400 font-semibold" id="topics-count">0</span> temas para generación de contenido</p>
    </div>
...
<div class="flex items-center gap-2 mb-5 flex-wrap">
    <button onclick="filterTopics('all')" data-filter="all"
            class="topic-filter active px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer
                   bg-white/10 text-white">Todos</button>
...
<div id="topics-loading" class="text-center py-20 text-gray-600">
    ...
    <h2 class="text-gray-400 text-lg font-semibold mb-2">Cargando temas...</h2>
</div>

<!-- Empty state -->
<div id="topics-empty" class="text-center py-20 text-gray-600 hidden">
    <div class="text-5xl mb-4">📝</div>
    <h2 class="text-gray-400 text-lg font-semibold mb-2">No hay temas todavía</h2>
    <p class="text-sm mb-6">Crea tu primer tema para empezar a generar contenido con AI.</p>
...
<div class="fixed inset-0 bg-black/80 z-[1100] hidden items-center justify-center backdrop-blur-sm" id="topicModal" onclick="closeTopicModal()">
    <div class="bg-[#16161e] rounded-2xl border border-surface-border w-[92%] max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl shadow-black/50"
        ...
        <form id="topicForm" onsubmit="saveTopic(event)" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1.5">Título *</label>
                <input type="text" id="topic-title" required placeholder="Ej: Tendencias de Marketing Digital 2026"
            ...
            <div>
                <label class="block text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1.5">Descripción</label>
                <textarea id="topic-description" rows="3" placeholder="Describe el tema y los puntos clave a cubrir..."
                          class="w-full bg-surface border border-surface-border rounded-xl px-4 py-2.5 text-sm text-white
                                 placeholder-gray-600 focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/30
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1.5">Categoría</label>
            ...
            <div>
                <label class="block text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1.5">Estado</label>
...
<div class="fixed inset-0 bg-black/80 z-[1200] hidden items-center justify-center backdrop-blur-sm" id="deleteModal" onclick="closeDeleteModal()">
    <div class="bg-[#16161e] rounded-2xl border border-red-500/20 w-[92%] max-w-sm shadow-2xl shadow-black/50 p-6 text-center"
        ...
        <h3 class="text-white font-semibold text-base mb-2">¿Eliminar este tema?</h3>
        <p class="text-gray-500 text-sm mb-5">Esta acción no se puede deshacer.</p>
...
function openTopicModal(topic) {
    ...
    document.getElementById('modal-title').lastChild.textContent = topic ? ' Editar Tema' : ' Nuevo Tema';
    document.getElementById('btn-save-topic').textContent = topic ? 'Actualizar' : 'Guardar';
...
function loadTopics() {
    fetch(API_URL)
        ...
        .catch(function(err) {
            document.getElementById('topics-loading').classList.add('hidden');
            document.getElementById('topics-empty').classList.remove('hidden');
            document.getElementById('topics-empty').querySelector('h2').textContent = 'Error al cargar';
...
function renderTopics() {
    ...
    var filtered = _currentFilter === 'all'
        ? _topics
        : _topics.filter(function(t) { return t.status === _currentFilter; });

    if (filtered.length === 0) {
        list.innerHTML = '<div class="col-span-full text-center py-12 text-gray-600">' +
            '<div class="text-3xl mb-2">🔍</div>' +
            '<p class="text-sm">No hay temas con este filtro</p></div>';
        return;
    }
        ...
        card.innerHTML =
            ...
            '<div class="mt-auto flex items-center justify-between pt-3 border-t border-surface-border">' +
                ...
                '<div class="flex items-center gap-1">' +
                    '<button onclick="openTopicModal(_topics[' + _topics.indexOf(topic) + '])" title="Editar" ' +
                    ...
                    '</button>' +
                    '<button onclick="openDeleteModal(' + topic.id + ')" title="Eliminar" ' +
